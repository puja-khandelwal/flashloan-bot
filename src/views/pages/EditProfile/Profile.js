    ...
    try {
      ...
      if (res.data && res.data.wallet) {
        setWalletData({
          ...
          minProfitPct: res.data.wallet.minProfitPct || 0.2,
          isLoading: false,
          isEditing: false,
        });
      } else {
...
const token = sessionStorage.getItem("token") || localStorage.getItem("creatturAccessToken");
      ...
      if (res.data) {
        // Toggle the current state (safer than relying on API response if it's inconsistent)
  ...
  const toggleWalletEditing = () => {
    setWalletData(prev => ({
      ...
      // Clear private key if canceling edit
            ...
            <Box className={classes.toggleContainer}>
              <Box
                className={`${classes.statusIndicator} ${
                  autoTradingEnabled ? classes.activeStatus : classes.inactiveStatus
                }`}
              />
